...
class SolutionTest {
    ...
    @TestEachSolver
    void traversesRightSkewedTreeInPreorder(SolutionVariants.Solver solver) {
        var three = new TreeNode(3);
        var two = new TreeNode(2, three, null);
        var root = new TreeNode(1, null, two);

        List<Integer> expected = List.of(1, 2, 3);
        assertEquals(expected, solver.preorderTraversal(root));
    }

    @TestEachSolver
    void traversesComplexTreeInPreorder(SolutionVariants.Solver solver) {
